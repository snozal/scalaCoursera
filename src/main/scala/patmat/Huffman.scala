...
object Huffman {
   ...
   */
    ...
    //http://stackoverflow.com/questions/11448685/scala-how-can-i-count-the-number-of-occurrences-in-a-list
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      ...
      isort(freqs).map(t => Leaf(t._1, t._2))
  }
  
  /**
   * Checks whether the list `trees` contains only one single code tree.
* Not very sure what i should be checking here.
   *  */
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    if(trees.size < 2) trees
      ...
      makeCodeTree(trees.head, trees.tail.head) :: combine(trees drop 2)
    }
  }
